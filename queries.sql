...
SELECT first_name, last_name, COUNT(vehicles.owner_id) FROM owners JOIN vehicles on owners.id=vehicles.owner_id GROUP BY (first_name, last_name) ORDER BY first_name;
...
GROUP BY 
  (first_name, last_name) 
HAVING 
  COUNT(vehicles.owner_id) > 1 AND ROUND(AVG(price)) > 10000 
